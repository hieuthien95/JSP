	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		case "forgot":
			...
			if (user != null) {
				...
				session.setAttribute("message", "Mật khẩu mới đã được gửi đến ");
			} else {
				session.setAttribute("message", "Không tồn tại ");
			}
			break;
		}
		...
		case "register":
			...
			if (new UserDAO().checUsername(username, email) == false) {
				new UserDAO().register(
						new User(0, username, sdt, email, ten, "Người dùng", avatar, MD5.encryption(password), 0));
				try {
					SendMail.sendMail(email, "DAJAVA", "Hello " + email + "\nYou sign up complete username: " + username
							+ ", password: " + password);
				} catch (Exception e) {
				}
			} else {
		...
		case "update":
			...
			if (user != null) {
				...
				if (MD5.encryption(password).equals(user.getPassword())) {
						...
						if (newpassword.equals(repassword)) {
							if (fileName.equals("")) {
								...
								new UserDAO().update(new User(stt, user.getUsername(), sdt, email, ten, quyen, avatar,
										MD5.encryption(newpassword), user.getDiemTichTuy()));
								response.sendRedirect("/UserServlet?command=logout");
								return;
							}
						} else {
							session.setAttribute("error", "Conform password is not valid");
						}
					} else {
						if (fileName.equals("")) {
							new UserDAO().update(new User(stt, user.getUsername(), sdt, email, ten, quyen,
					...
					session.setAttribute("error", "Password is not valid");
